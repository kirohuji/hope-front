...
export const AUDIT_SERVICE_OPTIONS = [...Array(8)].map((_, index) => ({
  id: _mock.id(index),
...
const ITEMS = [...Array(3)].map((__, index) => {
  const total = AUDIT_SERVICE_OPTIONS[index].price * _mock.number.nativeS(index);
  ...
  return {
    ...
    price: AUDIT_SERVICE_OPTIONS[index].price,
    service: AUDIT_SERVICE_OPTIONS[index].name,
    quantity: _mock.number.nativeS(index),
  };
});

export const _audits = [...Array(20)].map((_, index) => {
