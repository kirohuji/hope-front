...
import { postService, messagingService } from 'src/composables/context-provider';

// ----------------------------------------------------------------------

export default function PostDetailsBookers({ onRefresh, participants }) {
  ...
  const handleDelete = async (item, index) => {
    try {
      ...
      await postService.deleteUser(currentParticipant);
  ...
  return (
    <Stack component={Card} direction="row" spacing={2} key={participant._id} sx={{ p: 3 }}>
      ...
      <Stack spacing={2} flexGrow={1}>
        ...
        />
        <Restricted to={['PostListPersonSignOrDelete']}>
