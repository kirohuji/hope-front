...
const BROAECAST_TYPE_OPTIONS = [
  ...
  { value: 'post', label: '消息公告' },
  { value: 'familyGathering', label: '社交聚会' },
  // { value: 'book', label: '阅读' },
];

export default function PostItem({ post, onView, onEdit, onDelete }) {
  ...
  const renderTexts = (
    <ListItemText
      ...
      secondary={
        <Link component={RouterLink} href={paths.dashboard.post.details(_id)} color="inherit">
          {label}
        </Link>
      }
