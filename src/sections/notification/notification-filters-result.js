...
export default function NotificationFiltersResult({
  filters,
  onFilters,
  //
  canReset,
  ...
  const handleRemoveNotificationGuide = (inputValue) => {
    ...
    <Stack spacing={1.5} {...other}>
      ...
      <Stack flexGrow={1} spacing={1} direction="row" flexWrap="wrap" alignItems="center">
        ...
        {!!filters.tourGuides.length && (
          <Block label="Notification guide:">
            {filters.tourGuides.map((item) => (
              <Chip
                ...
                label={item.name}
                onDelete={() => handleRemoveNotificationGuide(item)}
              />
            ))}
          </Block>
        )}
...
NotificationFiltersResult.propTypes = {
