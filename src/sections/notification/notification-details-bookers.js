...
import { notificationService, messagingService } from 'src/composables/context-provider';

// ----------------------------------------------------------------------

export default function NotificationDetailsBookers({ onRefresh, participants }) {
  ...
  const handleDelete = async (item, index) => {
    try {
      ...
      await notificationService.deleteUser(currentParticipant);
  ...
  const signIn = useCallback(
    async (item, index) => {
      try {
        setLoading(true);
        setCurrentIndex(index);
        await notificationService.signIn(item);
...
NotificationDetailsBookers.propTypes = {
  ...
  return (
    <Stack component={Card} direction="row" spacing={2} key={participant._id} sx={{ p: 3 }}>
      ...
      <Stack spacing={2} flexGrow={1}>
        ...
        />
        <Restricted to={['NotificationListPersonSignOrDelete']}>
