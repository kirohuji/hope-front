...
export default function BroadcastFilters({
  ...
  const handleFilterBroadcastGuide = useCallback(
    (newValue) => {
  ...
  const renderDateRange = (
    <Stack>
      ...
      <Stack spacing={2.5}>
        <DatePicker label="Start date" value={filters.startDate} onChange={handleFilterStartDate} renderInput={()=>{}}/>
  ...
  const renderBroadcastGuide = (
    <Stack>
      ...
      <Autocomplete
        ...
        onChange={(event, newValue) => handleFilterBroadcastGuide(newValue)}
        getOptionLabel={(option) => option.name}
        renderInput={(params) => <TextField placeholder="Select Broadcast Guides" {...params} />}
