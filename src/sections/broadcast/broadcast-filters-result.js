...
export default function BroadcastFiltersResult({
  filters,
  onFilters,
  //
  canReset,
  ...
  const handleRemoveBroadcastGuide = (inputValue) => {
          ...
          <Block label="Broadcast guide:">
            {filters.tourGuides.map((item) => (
              <Chip
                ...
                label={item.name}
                onDelete={() => handleRemoveBroadcastGuide(item)}
              />
            ))}
          </Block>
        )}
...
BroadcastFiltersResult.propTypes = {
