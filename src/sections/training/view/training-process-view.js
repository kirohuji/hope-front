...
} from 'src/redux/slices/trainning';
// components
      ...
      <TabPanel value={scrollable} index="one" key={0}>
        ...
        >
          <Box sx={{ display: 'flex', position: 'relative' }}>
            {books.length ? (
              books.map((book, index) => <TrainingCard post={book} index={index} key={index} />)
            ) : (
              <TrainingCard post={defaultBook} />
            )}
            {false && (
              <Iconify
          ...
          {books.length ? (
            <Box sx={{ padding: '15px' }}>
              ...
              >
                <div>已完成 {bookSummarize.inProcess}天</div>
            ...
            <Box>
              <Grid container spacing={3}>
                <Grid item xs={6} sm={3} md={3}>
                  ...
                  >
                    <span style={{ fontWeight: 'bold', fontSize: '18px' }}>69</span>
                    <span style={{ fontSize: '14px', marginLeft: '4px' }}>分钟</span>
                  </div>
                </Grid>
                <Grid item xs={6} sm={3} md={3}>
                  <Label
                  ...
                  >
                    <span style={{ fontWeight: 'bold', fontSize: '18px' }}>5705</span>
                    <span style={{ fontSize: '14px', marginLeft: '4px' }}>分钟</span>
                  </div>
                </Grid>
              </Grid>
            ...
            <StaticDatePicker
              ...
              renderDay={(date, selectedDates, pickersDayProps) =>
                renderPickerDay(
                  ...
                  selectedDates,
                  pickersDayProps,
                  bookSummarize.days?.map((day) => new Date(day)) || []
                )
              }
              showToolbar={false}
              renderInput={() => null}
        ...
        >
          <div
            style={{
              backgroundImage:
                'url(https://api-dev-minimal-v4.vercel.app/assets/images/covers/cover_1.jpg)',
