...
const BROAECAST_TYPE_OPTIONS = [
  ...
  { value: 'membership', label: '消息公告' },
  { value: 'familyGathering', label: '社交聚会' },
  // { value: 'book', label: '阅读' },
];

export default function MembershipItem({ membership, onView, onEdit, onDelete }) {
  ...
  const renderTexts = (
    <ListItemText
      ...
      secondary={
        <Link component={RouterLink} href={paths.dashboard.membership.details(_id)} color="inherit">
          {label}
        </Link>
      }
  ...
  return (
    <>
      ...
      >
        ...
        {lgUp && (
          <Restricted to={['MembershipListEdit']}>
            <MenuItem
              onClick={() => {
                popover.onClose();
                onEdit();
        ...
        {lgUp && (
          <Restricted to={['MembershipListDelete']}>
            <MenuItem
              onClick={() => {
                popover.onClose();
                onDelete();
