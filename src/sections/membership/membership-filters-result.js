...
export default function MembershipFiltersResult({
  filters,
  onFilters,
  //
  canReset,
  ...
  const handleRemoveMembershipGuide = (inputValue) => {
          ...
          <Block label="Membership guide:">
            {filters.tourGuides.map((item) => (
              <Chip
                ...
                label={item.name}
                onDelete={() => handleRemoveMembershipGuide(item)}
              />
            ))}
          </Block>
        )}
...
MembershipFiltersResult.propTypes = {
