...
import { membershipService, userService, fileService } from 'src/composables/context-provider';
...
export default function MembershipNewEditForm({ currentMembership }) {
  const loading = useBoolean(false);

  const router = useRouter();

  const isEdit = !!currentMembership;
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      // images: currentMembership?.images || [],
      // type: currentMembership?.type || '',
      // //
      // leaders: currentMembership?.leaders || [],
      // // tags: currentMembership?.tags || [],
      // durations: currentMembership?.durations || '',
      // destination: currentMembership?.destination || '',
      // published: currentMembership?.published || false,
      // // services: currentMembership?.services || [],
      // available: {
      //   startDate: currentMembership?.available.startDate || null,
      //   endDate: currentMembership?.available.endDate || null,
      // },
    }),
    [currentMembership]
  );
  ...
  }, [currentMembership, defaultValues, reset]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      reset();
      enqueueSnackbar(currentMembership ? '更新成功!' : '创建成功!');
    ...
    <>
      ...
      <Grid xs={12} md={8} sx={{ display: 'flex', justifyContent: 'right' }}>
        ...
        <Button
          color="error"
          variant="contained"
          onClick={() => router.push(paths.dashboard.membership.root)}
          sx={{ mr: 1 }}
        >
          返回
        </Button>
...
MembershipNewEditForm.propTypes = {
