...
const slice = createSlice({
  ...
  reducers: {
    ...
    addParticipantsSuccess(state, action) {
      ...
      state.isLoading = false;
      if (state.details.byId[id]) {
        state.details.participantsBy[id] = _.unionBy(state.details.participantsBy[id], datas, "user_id")
      }
    },
    ...
    deleteParticipantSuccess(state, action) {
      const { id, data } = action.payload;
      if (state.details.byId[id]) {
        state.details.participantsBy[id] = state.details.participantsBy[id].filter(participant => participant.user_id !== data.user_id);
        state.details.count[id] = state.details.participantsBy[id]?.length;
      }
    },
...
export function addParticipants({
  ...
  return async (dispatch) => {
    try {
      dispatch(slice.actions.addParticipantsSuccess({
        id,
        datas,
      }));
    } catch (error) {
...
export function getParticipantsCount(id) {
  return async (dispatch, getState) => {
    try {
      ...
      //   dispatch(slice.actions.startLoading());
      // }
      dispatch(slice.actions.localParticipantsCountSuccess({
        id
      }));
      dispatch(slice.actions.startLoading());
      ...
      dispatch(slice.actions.getParticipantsCountSuccess({
        id,
        data: response
      }));
    } catch (error) {
...
export function deleteParticipant({
  ...
  return async (dispatch) => {
    try {
      dispatch(slice.actions.deleteParticipantSuccess({
        id,
        data
      }));
    } catch (error) {
...
export function updateParticipantStatus({
  ...
  return async (dispatch) => {
    try {
      dispatch(slice.actions.updateParticipantStatusSuccess({
        id,
        data,
        status
      }));
